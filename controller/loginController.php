...
class LoginController extends ControladorBase {
  ...
  public function index() {
    //echo $this->secury->hashing('ROOT777777',''); die;
    $this->view("login", [
       "title" => "login", 
       "sia" => "Sistema Integral Administrativo",
       "ctl" => "login", 
       "acc" => "validar",
      ]
    );
    ...
    if( isset($_POST['nick_user']) && isset($_POST['pswd_user']) && !empty(trim($_POST['nick_user'])) && !empty(trim($_POST['pswd_user']))) :
     ...
     $datasUser = $objUsuario->getDatasUsuario($usuario[0]->id);
      ...
      if ( !is_bool($usuario) ){        
          ...
          if ( $user_pwd_success==1 && $user_tkn_success==1 && $datasUser->type==='S') {
              ...
              setcookie ("us_SIA", $_SESSION['usuario']['nick'], 0, "/"," ",0);
          } else
            $process = FALSE;
      } else
        $process = FALSE; 
